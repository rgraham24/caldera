...
export default async function MusicPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 md:px-6 lg:px-8">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-[var(--text-primary)]">🎵 Music Markets</h1>
        <p className="mt-1 text-sm text-[var(--text-tertiary)]">
          From chart positions to tour announcements
        </p>
      </div>

      {items.length === 0 ? (
        <p className="py-16 text-center text-sm text-[var(--text-tertiary)]">No music markets open right now.</p>
      ) : (
