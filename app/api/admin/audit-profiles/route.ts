...
import { auditAndFixReservedProfiles } from '@/lib/admin/pipeline';
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { fixed, removed } = await auditAndFixReservedProfiles(supabase, 100);
    return NextResponse.json({ fixed, removed });
  } catch (err) {
